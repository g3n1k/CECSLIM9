...
$query = "SELECT biblio.title as title
          FROM biblio
          WHERE biblio.title LIKE '%$word%'";
...
$query = "SELECT biblio.call_number as cm
          FROM biblio
          WHERE biblio.call_number LIKE '%$word%'";
$a = $dbs->query($query);
foreach ($a as $value) {
  if ($no > 0) {
    $isi .= ', ';
...
$query = "SELECT mst_publisher.publisher_name as pna
          FROM biblio
          JOIN mst_publisher ON biblio.publisher_id = mst_publisher.publisher_id
