...
if (isset($_POST['save']) AND (isset($_POST['authorID']) OR trim($_POST['search_str']))) {
    ...
    if (isset($_POST['biblioID']) AND !empty($_POST['biblioID'])) {
        ...
        if ($sql_op->insert('korupsi_author', $data)) {
            echo '<script type="text/javascript">';
            echo 'alert(\''.__('Author succesfully updated!').'\');';
            echo 'parent.setIframeContent(\'authorIframe\', \''.MWB.'korupsi/iframe_author.php?biblioID='.$data['biblio_id'].'\');';
            echo '</script>';
        } else {
        ...
        echo 'parent.setIframeContent(\'authorIframe\', \''.MWB.'korupsi/iframe_author.php\');';
        echo '</script>';
    }
}
