...
if (isset($_POST['bid']) AND isset($_POST['remove'])) {
    ...
    $sql_op->delete('korupsi_attachment', "file_id=$file AND biblio_id=$bid");
...
if ($biblioID) {
    ...
    // database list
    $biblio_attach_q = $dbs->query('SELECT att.*,fl.* FROM korupsi_attachment AS att
    ...
    while ($biblio_attach_d = $biblio_attach_q->fetch_assoc()) {
        ...
        // remove link
        $remove_link = '<a href="#" onclick="confirmProcess('.$biblioID.', '.$biblio_attach_d['file_id'].', \''.addslashes($biblio_attach_d['file_name']).'\')" class="btn btn-danger">Delete</a>';

        // edit link
        $edit_link = '<a href="#" onclick="top.openHTMLpop(\''.MWB.'korupsi/pop_attach.php?biblioID='.$biblioID.'&fileID='.$biblio_attach_d['file_id'].'\', 600, 300, \''.__('File Attachments').'\')" class="btn btn-default">Edit</a>';

        // file link
        if (preg_match('@(video|audio|image)/.+@i', $biblio_attach_d['mime_type'])) {
            $file = '<a class="notAJAX openPopUp" href="'.SWB.'index.php?p=multimediastream&fid='.$biblio_attach_d['file_id'].'&bid='.$biblio_attach_d['biblio_id'].'" title="'.$biblio_attach_d['file_title'].'">'.$biblio_attach_d['file_title'].'</a>';
        } else {
