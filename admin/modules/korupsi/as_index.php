...
<fieldset class="menuBox">
  <div class="menuBoxInner masterFileIcon">
  	<div class="per_title">
      	<h2><?php echo __('Activity Sarasehan Index'); ?></h2>
  	</div>
  </div>
</fieldset>
      ...
      <div class="col-md-12">
        <div class="panel panel-info">
          <div class="panel-body" style="min-height:470px;">
            <section style="display: inline;"><?php echo __('Search'); ?> :
            ...
            <section class="indexing">
              <table align="center"  cellpadding="5" cellspacing="0" style="width:100%;">
                <tr class="dataListHeader" style="font-weight: bold; cursor: pointer; background-color: rgb(49, 53, 62);">
                  <td></td>
                  <td class="sort" data-value="ss_title" data-sort="DESC"><?php echo __('Title'); ?></td>
                  <td class="sort" data-value="ss_desk" data-sort="DESC"><?php echo __('Description'); ?></td>
                  <td class="sort" data-value="tgl_update" data-sort="DESC"><?php echo __('Update Date'); ?></td>
                </tr>
                <tbody class="isian">
            ...
            <section class="editing" style="display:none;">
              <form id="2" action="javascript:void(0)" method="post">
                <table>
                  <tr>
                    <td><?php echo __('Title'); ?></td>
                    <td>
                      <input type="hidden" class="id" style="width:100%" name="id" value="">
                      <input type="text" class="judul" style="width:100%" name="title" value="">
                    </td>
                  </tr>
                  <tr>
                    <td><?php echo __('Picture'); ?></td>
                    <td>
                      <input type="hidden" class="plma" style="width:100%" name="pic_lama" value="">
                      <input type="file" class="gbr" name="pile" value=""><br>
                      <section class="ima">
                  ...
                  <tr>
                    <td></td>
                    <td>
                      <input type="submit" name="" value="<?php echo __('Save'); ?>" class="btn btn-primary button simpan">
                      <input type="reset" name="" value="<?php echo __('Cancel'); ?>" class="btn btn-success button batal">
                    </td>
                  </tr>
                </table>
              </form>
...
<a class="cobaan" href="<?php echo $_SERVER["REQUEST_URI"];?>" title="Edit">
...
$(document).ready(
  ...
  $(document).on('click','.alcari',function() {
    keyword = $('.alfind').val();
    paging();
    buku();
  })
  ...
  $('.isian').on('click','.dele',function() {
    idnya = $(this).data('id');
    $('.def-'+idnya).toggle();
  })
  $('.isian').on('click','.ed',function() {
    id = $(this).data('id');
    $.ajax({
      type : 'POST',
      url : '<?php echo MWB; ?>korupsi/as_pdata.php',
      dataType : 'json',
      data : {
        'id' : id
      },
      success : function(data) {
        ...
        $('.id').val(data.id);
        $('.plma').val(data.pic);
        CKEDITOR.instances['DSC'].setData(data.desk);
        if (data.pic == null) {
          $('.ima').empty();
        }else {
          $('.ima').empty();
          $('.ima').append(
            '<img src="../images/sarasehan/'+data.pic+'" style="max-width:200px; max-height:200px;">'
          );
        }
        $('.indexing').slideUp();
        $('.editing').slideDown();
  ...
  $(document).on('click','.submenu-12',function() {
    $('.indexing').slideDown();
    $('.editing').slideUp();
  })
  ...
  $(document).on('click','.del',function() {
    ...
    $.ajax({
      type : 'POST',
      url : '<?php echo MWB; ?>korupsi/as_del.php',
      data : {
        'id' : id
      },success : function(data) {
        paging();
  ...
  $( '#2' ).submit( function( e ) {
    ...
    if (!title && !deskripsi) {
      ...
      if (!file) {
        $.ajax({
          url: '<?php echo MWB; ?>korupsi/as_update.php',
          type: 'POST',
  ...
  function paging() {
    $.ajax({
      type : 'POST',
      url : '<?php echo MWB; ?>korupsi/as_paging.php',
      dataType : 'JSON',
      data : {
        'keyword':keyword,
        'fieldnya':fieldnya,
  ...
  function buku() {
    $.ajax({
      type : 'POST',
      url : '<?php echo MWB; ?>korupsi/as_data.php',
      data : {
        'keyword' : keyword,
        'page' : page,
        'sortnya': sortnya,
