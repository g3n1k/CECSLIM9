...
$file = $_FILES['pile'];
$nmfile = $_FILES['pile']['name'];

$plma = $_POST['pic_lama'];

if (empty($nmfile)) {
  $tgl_buat = date('Y-m-d');
  $query = "UPDATE sarasehan SET ss_title = '$title',
                                  ss_desk = '$deskripsi',
                                  tgl_update = '$tgl_buat'
                                  WHERE id = '$id'";
  ...
  $cek_display  = $dbs->query("SELECT * FROM sarasehan_display WHERE img = '$plma'");
  if (!empty($cek_display)) {
    $query_display = "UPDATE sarasehan_display SET img = '$filenya' WHERE img = '$plma'";
    $up_display = $dbs->query($query_display);
  }
}
