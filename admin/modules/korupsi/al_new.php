...
<div class="menuBoxInner masterFileIcon">
	<div class="per_title">
    	<h2><?php echo __('Input Book Activity'); ?></h2>
	</div>
      ...
      <div class="col-md-12">
        <div class="panel panel-info">
          <div class="panel-body" style="min-height:470px;">
            <section style="display: inline;"><?php echo __('Barcode ID'); ?> :
              <input class="isbn" type="text" name="keywords" size="30" />
              <input type="submit" value="<?php echo __('Check'); ?>" class="button btn btn-primary cek" />
            </section>
            <hr>
            <form class="sform" action="<?php echo MWB; ?>korupsi/al_s_buku.php" method="post">
              <table align="center" id="dataList" cellpadding="5" cellspacing="0" style="display:none">
              ...
              <br>
              <input type="submit" value="<?php echo __('Save'); ?>" class="button btn btn-primary save pull-right" style="display:none;" />
            </form>
          </div>
        </div>
      </div>
...
<script type="text/javascript">
 ...
 var jmldata = 0;
  ...
  $('.baca').on('click','.cek',function() {
    isbn = $('.isbn').val();
    bdata();
  })
  $('.baca').on('click','.hapus',function() {
    var nomer = $(this).data('value');
    jmldata = jmldata - 1;
    $('.'+nomer).remove();
    if (jmldata < 1) {
      $('#dataList').hide();
  ...
  function bdata() {
    if (!isbn) {
      alert('Barcode Field Cannot Empty');
    } else {
      $.ajax({
        type : 'POST',
        url : '<?php echo MWB; ?>korupsi/al_f_buku.php',
        dataType : 'JSON',
        data : {
          'isbn' : isbn,
        },
        success:function(data) {
          if (data.jml > 0) {
            ...
            jmldata = jmldata + 1;
            $('.isian').append(
  						'<tr class="alterCell '+no+'">' +
                '<input type="hidden" name="id[]" value="'+data.id+'">'+
                '<td align="center"><h4 style="color:red;cursor:pointer;" class="hapus" data-value="'+no+'"><b>X</b></h4></td>'+
  							'<td>'+data.item+'</td>'+
  							'<td>'+data.isbn+'</td>'+
  							'<td>'+data.id+'</td>'+
  							'<td>'+data.title+'</td>'+
            ...
            $('.save').show();
              ...
              alert('System cannot find related data')
          }
        }
      })
    }
