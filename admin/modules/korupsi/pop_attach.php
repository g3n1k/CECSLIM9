...
if (isset($_POST['upload']) AND trim(strip_tags($_POST['fileTitle'])) != '') {
    ...
    if (isset($_FILES['file2attach']) AND $_FILES['file2attach']['size']) {
        ...
        $file_upload->setUploadDir(REPOBS.DIRECTORY_SEPARATOR.str_replace('/', DIRECTORY_SEPARATOR, $file_dir));
        ...
        if ($file_upload_status === UPLOAD_SUCCESS) {
            ...
            utility::writeLogs($dbs, 'staff', $_SESSION['uid'], 'Konten Lokal', $_SESSION['realname'].' upload file ('.$file_upload->new_filename.')');
        } else {
    ...
    if (isset($_POST['updateBiblioID']) AND !empty($_POST['updateBiblioID'])) {
        ...
        if (isset($_POST['updateFileID'])) {
            ...
            $update1 = $sql_op->update('korupsi_attachment', array('access_type' => $data['access_type'], 'access_limit' => $data['access_limit']), 'biblio_id='.$updateBiblioID.' AND file_id='.$fileID);
            ...
            if ($update1) {
                ...
                echo 'parent.setIframeContent(\'attachIframe\', \''.MWB.'korupsi/iframe_attach.php?biblioID='.$updateBiblioID.'\');';
                echo '</script>';
            } else {
                utility::jsAlert(''.__('File Attachment data FAILED to update!').''."\n".$sql_op->error);
            }
        } else {
            if ($sql_op->insert('korupsi_attachment', $data)) {
                echo '<script type="text/javascript">';
                echo 'alert(\''.__('File Attachment uploaded succesfully!').'\');';
                echo 'parent.setIframeContent(\'attachIframe\', \''.MWB.'korupsi/iframe_attach.php?biblioID='.$data['biblio_id'].'\');';
                echo '</script>';
            } else {
                utility::jsAlert(''.__('File Attachment data FAILED to save!').''."\n".$sql_op->error);
            }
        }
        utility::writeLogs($dbs, 'staff', $_SESSION['uid'], 'Konten Lokal', $_SESSION['realname'].' updating file attachment data');
    } else {
        if ($uploaded_file_id) {
            ...
            echo '<script type="text/javascript">';
            echo 'alert(\''.__('File Attachment uploaded succesfully!').'\');';
            echo 'parent.setIframeContent(\'attachIframe\', \''.MWB.'korupsi/iframe_attach.php\');';
            echo '</script>';
        }
    }
}
...
$file_attach_q = $dbs->query("SELECT fl.*, batt.* FROM files AS fl
    LEFT JOIN korupsi_attachment AS batt ON fl.file_id=batt.file_id
