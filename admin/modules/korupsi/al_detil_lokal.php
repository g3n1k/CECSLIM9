...
$query = "SELECT korupsi.isbn_issn as isbn_issn,
          ...
          (SELECT COUNT(id_pengunjung) FROM pengunjung WHERE pengunjung.biblio_id = korupsi.biblio_id) as visitor,
          (SELECT COUNT(comment_id) FROM comments WHERE comments.biblio_id = korupsi.biblio_id) as komentar,
          (SELECT COUNT(id_share) FROM sharing WHERE sharing.content_id = korupsi.biblio_id) as sharing,
          (SELECT COUNT(id) FROM notif_comments WHERE notif_comments.biblio_id = korupsi.biblio_id) as ncm,
          (SELECT COUNT(id_share) FROM sharing WHERE sharing.content_id = korupsi.biblio_id AND sharing.sosmed = 'facebook') as fb,
          (SELECT COUNT(id_share) FROM sharing WHERE sharing.content_id = korupsi.biblio_id AND sharing.sosmed = 'twitter') as tw,
          (SELECT COUNT(id_share) FROM sharing WHERE sharing.content_id = korupsi.biblio_id AND sharing.sosmed = 'google') as goo,
          (SELECT COUNT(id_share) FROM sharing WHERE sharing.content_id = korupsi.biblio_id AND sharing.sosmed = 'linkedin') as lin,
          (SELECT COUNT(id_share) FROM sharing WHERE sharing.content_id = korupsi.biblio_id AND sharing.sosmed = 'tumblr') as tum,
          (SELECT COUNT(id_share) FROM sharing WHERE sharing.content_id = korupsi.biblio_id AND sharing.sosmed = 'whatsapp') as wha
          FROM korupsi
          LEFT JOIN item ON korupsi.biblio_id = item.biblio_id
          WHERE korupsi.biblio_id LIKE '%$id%'
          ";
