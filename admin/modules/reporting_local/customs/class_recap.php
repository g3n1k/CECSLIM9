...
?>
    ...
    <div id="filterForm">
        <div class="divRow">
            ...
            $recapby_options[] = array('owner_id', __('Owner'));
    ...
    if (isset($_GET['recapBy']) AND trim($_GET['recapBy']) != '') {
        switch ($_GET['recapBy']) {
            ...
            while ($gmd_d = $gmd_q->fetch_row()) {
                ...
                // count by title
                $bytitle_q = $dbs->query("SELECT COUNT(biblio_id) FROM korupsi WHERE gmd_id=".$gmd_d[0]);
                $bytitle_d = $bytitle_q->fetch_row();
                $output .= '<td class="'.$row_class.'"><strong style="font-size: 1.3em;">'.$bytitle_d[0].'</strong></td>';
                // count by item
                $byitem_q = $dbs->query("SELECT COUNT(item_id) FROM item AS i INNER JOIN korupsi AS b
            ...
            while ($lang_d = $lang_q->fetch_row()) {
                ...
                // count by title
                $bytitle_q = $dbs->query("SELECT COUNT(biblio_id) FROM korupsi WHERE language_id='".$lang_d[0]."'");
                $bytitle_d = $bytitle_q->fetch_row();
                $output .= '<td class="'.$row_class.'"><strong style="font-size: 1.3em;">'.$bytitle_d[0].'</strong></td>';
            ...
            while ($ctype_d = $ctype_q->fetch_row()) {
                ...
                $bytitle_q = $dbs->query("SELECT DISTINCT biblio_id FROM item AS i
                    WHERE i.biblio_id IN (SELECT biblio_id FROM korupsi) AND i.coll_type_id=".$ctype_d[0]."");
        ...
        while ($class_num < 10) {
            ...
            // count by title
            $bytitle_q = $dbs->query("SELECT COUNT(biblio_id) FROM korupsi WHERE TRIM(classification) LIKE '$class_num%'");
            $bytitle_d = $bytitle_q->fetch_row();
            $output .= '<td class="'.$row_class.'"><strong style="font-size: 1.5em;">'.$bytitle_d[0].'</strong></td>';

            // count by item
            $byitem_q = $dbs->query("SELECT COUNT(item_id) FROM item AS i LEFT JOIN korupsi AS b
                ON i.biblio_id=b.biblio_id
                WHERE i.biblio_id IN (SELECT biblio_id FROM korupsi) AND TRIM(b.classification) LIKE '$class_num%'");
            $byitem_d = $byitem_q->fetch_row();
            ...
            while ($class_num2 < 10) {
                ...
                // count by title
                $bytitle_q = $dbs->query("SELECT COUNT(biblio_id) FROM korupsi WHERE TRIM(classification) LIKE '".$class_num.$class_num2."%'");
                $bytitle_d = $bytitle_q->fetch_row();
        ...
        // count by title
        $bytitle_q = $dbs->query("SELECT COUNT(biblio_id) FROM korupsi WHERE TRIM(classification) LIKE '2X%'");
        $bytitle_d = $bytitle_q->fetch_row();
        $output .= '<td class="'.$row_class.'"><strong style="font-size: 1.5em;">'.$bytitle_d[0].'</strong></td>';
        ...
        $_non_decimal_q = $dbs->query("SELECT DISTINCT classification FROM korupsi WHERE classification REGEXP '^[^0-9]'");
        if ($_non_decimal_q->num_rows > 0) {
            while ($_non_decimal = $_non_decimal_q->fetch_row()) {
                ...
                // count by title
                $bytitle_q = $dbs->query("SELECT COUNT(biblio_id) FROM korupsi WHERE classification LIKE '".$_non_decimal[0]."'");
                $bytitle_d = $bytitle_q->fetch_row();
                $output .= '<td class="'.$row_class.'"><strong style="font-size: 1.5em;">'.$bytitle_d[0].'</strong></td>';
