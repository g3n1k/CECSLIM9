...
<div class="menuBoxInner statisticIcon">
	<div class="per_title">
	  <h2><?php echo __('Report Per Book'); ?></h2>
  </div>
	<!-- <div class="infoBox">
      ...
      <div class="col-md-12">
        <div class="panel panel-info">
          <div class="panel-body" style="min-height:470px;">
            <section class="lb_indexnya lb">
              <section class="search_lb" style="display: inline;"><?php echo __('Search'); ?>
                <br>
                <input class="lbfind" type="text" name="keywords" size="30" />
                <!-- <select name="field">
                ...
                <input type="submit" value="<?php echo __('Show More Filter Options'); ?>" class="button btn btn-primary lbfil" />
                <input type="submit" value="<?php echo __('Apply Filter'); ?>" class="button btn btn-primary lbcari" />
              </section>
              <hr>
              <div class="biblioPaging pull-right">
                <span class="pagingList lis_lb">
                </span>
              </div>
              <br>
              <hr>
              <section class="indexing">
                <table align="center"  cellpadding="5" cellspacing="0" style="width:100%;">
                  <tr class="dataListHeader" style="font-weight: bold; cursor: pointer; background-color: rgb(49, 53, 62);">
                    <td></td>
                    <td style="cursor:pointer;" class="sort" data-value="ti" data-sort="DESC"><?php echo __('Title'); ?></td>
                    <td style="cursor:pointer;" class="sort" data-value="read" data-sort="DESC"><?php echo __('Read'); ?></td>
                    <td style="cursor:pointer;" class="sort" data-value="visit" data-sort="DESC"><?php echo __('Visit'); ?></td>
                    <td style="cursor:pointer;" class="sort" data-value="jlo" data-sort="DESC"><?php echo __('Loan times'); ?></td>
                    <td style="cursor:pointer;" class="sort" data-value="al" data-sort="DESC"><?php echo __('Active Loan'); ?></td>
                  </tr>
                  <tbody class="is_lb">
            ...
            <section class="lb_detilnya lb" style="display:none;">
              <button type="button" name="button" class="lb_back button btn btn-primary"><?php echo __('Back to List'); ?></button>
              ...
              <hr>
              <section id="lb_lb_lb" class="lb_lb_lb">
              </section>
            </section>
...
<script type="text/javascript">
  ...
  $('.lbfil').click(function() {
    $('.filter').slideToggle();
  })
  $('.is_lb').on('click','.lb_detil',function(e) {
    $('.lb').slideToggle();
    $('.tile').empty();
    $('.tile').append($(this).data('title'));
    biblio_id = $(this).data('id');
    lb_detail();
  })
  $('.lb_detilnya').on('click','.lb_back',function(e) {
    $('.lb').slideToggle();
  })
  $('.search_lb').on('click','.lbcari',function(e) {
    cari = '%'+$('.lbfind').val()+'%';
    tgl_awal = $('.awal').val();
    tgl_akhir = $('.akhir').val();
    halaman = '1';
  ...
  $('.lis_lb').on('click','.pagingnya',function(e) {
  ...
  function lb_detail() {
    $.ajax({
      type : 'POST',
      url : '<?php echo MWB; ?>reporting_local/lb_data_detil.php',
      data :{
        'biblio_id' : biblio_id,
        'tgl_awal' : tgl_awal,
        'tgl_akhir' : tgl_akhir,
  ...
  function lb() {
    $.ajax({
      type : 'POST',
      url : '<?php echo MWB; ?>reporting_local/lb_data_index.php',
      data : {
        'cari' : cari,
        'urut' : urut,
        'tgl_awal' : tgl_awal,
  ...
  function lbp() {
    $.ajax({
      type : 'POST',
      url : '<?php echo MWB; ?>reporting_local/lb_data_paging.php',
      data :{
        'cari' : cari,
        'urut' : urut,
        'tgl_awal' : tgl_awal,
...
var url = "<?php echo AWB.'modules/reporting_local/lb_excel.php';?>";
/* */	
var $iframe,
      iframe_doc,
      iframe_html;
